...
public class DoorManagerMiraHQ : MonoBehaviourPunCallbacks
{
    //Creating an instance of this door manager
    public static DoorManagerMiraHQ Instance;
    ...
    private void RPC_OpenDoor(int index)
    {
        ...
        if (index == 0 || index == 1)
        {
            for (int i = 0; i < ParticleSystemManager.Instance.smokeDecontamination.Length; i++)
            {
                ParticleSystemManager.Instance.smokeDecontamination[i].Play();
            }
        }
    }
