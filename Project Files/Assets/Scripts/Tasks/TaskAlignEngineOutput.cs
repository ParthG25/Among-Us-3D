...
public class TaskAlignEngineOutput : MonoBehaviour
{
    ...
    //gameObjects holding the lever and the radar
    public Slider lever;
    public GameObject radar;

    private bool started, taskCompleted;

    private void Awake()
    {
        lever.value = Random.Range(0.8f, 1f);
        radar.transform.rotation = Quaternion.Euler(0f, 0f, 45 + (90 * lever.value));
    }
    ...
    private void Update()
    {
        ...
        if (TaskManager.Instance.activeTasks[serialNumber] && !taskCompleted)
        {
            ...
            if(lever.value > 0.45f && lever.value < 0.55f)
            {
                lever.interactable = false;
                lever.value = 0.5f;
                radar.transform.rotation = Quaternion.Euler(0f, 0f, 0f);
