...
public class TaskInsertKeys : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (Input.GetMouseButtonUp(0) && keys.initialized)
            {
                keys.canvasGroup.alpha = 1f;
                keys.canvasGroup.blocksRaycasts = true;
                keys.rectTransform.anchoredPosition = keys.position;
            }
            
            if(keyHole.keyReceived)
            {
