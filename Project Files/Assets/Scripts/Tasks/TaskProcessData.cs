...
public class TaskProcessData : MonoBehaviour
{
    ...
    IEnumerator FinishTask()
    {
        ...
        while (timer < 20f)
        {
            ...
            progress.text = ((int)(timer / 20f * 100)) + "% completed";
            file.GetComponent<RectTransform>().anchoredPosition = Vector3.MoveTowards(file.GetComponent<RectTransform>().anchoredPosition, new Vector3(550f, 0f, 0f), 300 * Time.deltaTime);

            if(file.GetComponent<RectTransform>().anchoredPosition.x >= 550f)
                file.GetComponent<RectTransform>().anchoredPosition = new Vector3(-500f, 0f, 0f);
